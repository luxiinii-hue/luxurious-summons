...
| **0.4.3** | Wire template thumbnails + inline-synthesized token textures to **Foundry CORE icons** (zero module dependency). Paths sourced from the dnd5e SRD spell entries (`packs/_source/spells/<level>/<name>.yml` `img:` field) — every Foundry install has these icons at `icons/...`. Mappings: Find Familiar + Pact of the Chain → `icons/creatures/birds/corvid-flying-wings-purple.webp`; Animate Dead → `icons/magic/control/fear-fright-monster-red.webp`; Mage Hand → `icons/magic/unholy/strike-hand-glow-pink.webp` (also token + portrait); Unseen Servant → `icons/creatures/magical/spirit-undead-masked-blue.webp` (also token + portrait); Echo Knight Echo → `icons/magic/control/debuff-energy-hold-levitate-pink.webp` (Mirror Image's SRD icon — analogous "illusory duplicate"; also token + portrait); Summon Dragon → `icons/magic/control/energy-stream-link-spiral-orange.webp` (stopgap — Summon Draconic Spirit isn't in dnd5e SRD; reused Simulacrum's icon). Simulacrum keeps its hand-authored `assets/templates-thumbs/simulacrum.svg` placeholder. Eliminates ~10 PNG 404s per gallery render. Variant thumbnails for compendium-sourced templates (Find Familiar variants etc.) still 404 — those clear up once UUIDs are wired (task #47) and Foundry pulls thumbnails from each creature's compendium `img` directly. |
