...
| **1.0.0** | **Plan 5 + 1.0.** (1) **Manager Settings tab is real** — per-user toggles for the Companion Bar (user request: "some players may not want the new switcher"), token effects, death animations, verbose logging. Same client-scope settings the settings sheet writes, so both stay in sync; `enablePIXIFilters` gained an `onChange` so toggling takes effect immediately instead of at next redraw. (2) **Heal-sweep C** retro-fixes companions spawned before 0.8.0 — they still carry `disposition: -1` and `actorLink: false` with no way for a player to discover or fix either. Repairs `prototypeToken` + every placed token across every scene; idempotent, and deliberately leaves a GM's hand-set Neutral/Secret disposition alone (only the un-normalized hostile default is rewritten). (3) **Aesthetic-family chrome (Plan 5)** — `aestheticFamily` was declared on 14 templates but expressed as exactly one border colour. Now three inherited custom properties (`--luxsum-family-accent`, `-accent-hi`, `-frame`) defined once in `luxurious.css` under `[data-family]`, consumed by swapping `var(--luxsum-accent)` → `var(--luxsum-family-accent)` at chosen accent points: variant-picker title, summon-details double-rule frame, companion-card thumb frames, GM-card template label, gallery card edge, and Companion Bar chip frames (brass vs arcane cyan). Because custom properties inherit, this needed **no** per-family selectors anywhere except the one palette block — a third family is one rule block. Restraint held: the wine-and-gold base chrome never varies, or the two families would read as two modules. The bar's controlled-state brightens the chip's OWN family frame rather than swapping in fixed gold, so a Hextech summon stays Hextech while selected. (4) `companion-bar.css` re-scoped from `#luxsum-companion-bar` to `.luxsum-bar` — ID scoping made the component impossible to preview side-by-side, and the class names are unique enough not to need the specificity. (5) **README rewritten** (was stale at v0.1.1 with manual-install steps). Previews: `previews/companion-bar.html`, `previews/family-chrome.html`. **Deliberately NOT built:** concentration/duration auto-expiry — the user's call, since dismissal via the Manager is already easy and auto-deleting an actor on a concentration hiccup is a worse failure mode than a manual click. |

| **1.0.1** | Two defects found by auditing the 1.0.0 claim "everything is in place". (1) **The GM's caps weren't caps.** `performSpawn` never called `checkRestrictions` — a known TODO since 0.4.6, comment-only for five releases. Per-template `maxActive`, the global per-player cap, and anti-spam were enforced ONLY on the requester's own client, so a stale index, two rapid casts racing the `recentSpawnTimestamps` write, or a modified client all got a spawn through uncontested. Now re-checked authoritatively on the executing client against live `game.actors` (not the user-flag index — the index is precisely what a wrong client is wrong about), and **batch-aware**: a request carrying N placements is validated by padding `activeCompanions` with the other N-1, so "would the last one still be allowed?" is the question asked. Unpadded, one request with 100 placements passed a single-spawn check and created 100 actors. GMs are exempt — they may overrule their own table. New suite `lux-batch-restrictions.test.js` pins the semantics. (2) **Family chrome didn't reach the live gallery.** 1.0.0 keyed the template-card left border on `data-family`, but the Manager's Spawn New tab renders `template-card.hbs`, which never carried the attribute — only the *dead* `template-gallery-card.hbs` did, so every card on the live path silently fell back to the default gilt. Attribute added. **Tests: 232 → 238.** Still-open, deliberately not acted on: `spawn-gallery-app.js` + `spawn-gallery.hbs` + `template-gallery-card.hbs` are dead code (nothing opens the app; `spawn-gallery.css` IS live — the Manager tab inherits `.luxsum-template-card` from it) — wire-vs-delete is a product call. |

**238 unit tests passing.** Distribution ZIPs in this repo's `dist/` (gitignored as of 2026-07-13; parent `../../dist/` was the original convention but has never actually held this module's ZIPs).
...
- **Testing the GM Console requires two sessions, and there is no shortcut.** Decided 2026-07-30 after evaluating an "easter egg" unlock (flip a client setting 5× → GM tabs appear for that player). **Rejected, do not re-propose.** Two reasons: (1) unlocking the tabs only changes what's *drawn* — every console control writes world-scope state (`gmMotionEnabled`, `gmMotionIntensity`, `templateOverrides`, `Actor.delete` for force-dismiss), and Foundry rejects those from a non-GM at the server, so the player gets a panel of dead switches; (2) making the test account Assistant GM *does* grant real rights (`User#isGM` is `role >= ASSISTANT`) but changes the very paths under test — this module branches on `game.user.isGM` for direct-spawn vs broker hand-off, primary-GM election, and the heal sweep. A relay-through-a-GM variant would work but is a hidden privilege-escalation backdoor in a publicly installable module. **Correct setup:** a normal window logged in as the player and a private window logged in as the GM, both on the same world — the GM client is also what executes the player's spawns, which the broker path needs anyway.
